...
class ChooseSheetThread(QtCore.QThread):
    ...
    def run(self):
        # self.header_row = 1
        global_vars.df = pd.read_excel(global_vars.file, header = None, dtype= 'string', engine = 'calamine', sheet_name = global_vars.sheet_name, nrows = 17)
    ...
    def on_finished(self): # Вызывается при завершении потока
        ...
        global_vars.ui.footer_label.setText(f'Несколько начальных строк листа "{global_vars.ui.comboSheets.currentText()}" загружены для предпросмотра!')
